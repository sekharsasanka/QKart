...
export default class Cart extends React.Component {
  ...
  refreshCart = async () => {
    ...
    if(this.state.items.length === 0){
       message.error('You must add products before going to checkout')
      ...
      >
        ...
        {!this.props.checkout && (
          <Button
            ...
            onClick={() => {
              this.state.items.length ? this.props.history.push('/checkout'):message.error('You must add items to cart first')
