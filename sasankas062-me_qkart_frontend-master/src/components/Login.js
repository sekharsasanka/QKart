...
class Login extends React.Component {
  ...
  login = async () => {
    if(this.validateInput() === true){
      let responseNew = await this.performAPICall();
      if(responseNew){
        await this.persistLogin(responseNew.token,responseNew.username,responseNew.balance);
        // console.log(responseNew.token);
        this.setState({
          username:"",
          password:""
        })
