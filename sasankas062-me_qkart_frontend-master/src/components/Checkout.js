...
class Checkout extends React.Component {
  ...
  deleteAddress = async (addressId) => {
    ...
    try{
      let res = await fetch (`${config.endpoint}/user/addresses/${addressId}`,{
        method:'DELETE',
        headers:{
          'authorization': `Bearer ${localStorage.getItem('token')}`,
          'content-type':'application/json'
  ...
  checkout = async () => {
    ...
    if (this.validateResponse(errored, response, "checkout")) {
        ...
        message.success('Your order has placed succesfully')
        let newBalance = this.state.balance - this.cartRef.current.calculateTotal()
  ...
  order = async () => {
    if(this.state.balance < this.cartRef.current.calculateTotal()){
      message.error('cannot proceed to checkout since the wallet balance is not sufficient to complete this operation')
    }else if(this.state.addresses.length === 0 || this.state.selectedAddressIndex === -1){
      message.error('Please select an address or add a new address')
    }else{
      await this.checkout()
    }
  };
  ...
  async componentDidMount() {
    if(localStorage.getItem('username') && localStorage.getItem('token')){
      ...
      message.error('You must be logged in first to visit the checkout page')
